...
export function EmbedPreview({ machineId }: { machineId: string }) {
  const src = `${window.location.origin}/embed/machine/${machineId}`;
  return (
...
export function EmbedSnippet({ machineId }: { machineId: string }) {
  const code = `<iframe title="Vending Machine" src="${window.location.origin}/embed/machine/${machineId}" style="width:340px;height:500px;border:0;" ></iframe>`;
  return (
    <div>
