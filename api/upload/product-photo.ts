...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  console.log('🔍 Product photo upload endpoint called');
  ...
  try {
    if (req.method === 'POST') {
      ...
      console.log('Product photo received:', {
      ...
      try {
        ...
        const uniqueFilename = `product_${timestamp}.${extension}`;
        ...
        console.log(`✅ Product photo uploaded to Vercel Blob: ${uniqueFilename} (${buffer.length} bytes)`);
        ...
        return res.status(500).json({
          error: 'Failed to upload product photo to Vercel Blob Storage',
